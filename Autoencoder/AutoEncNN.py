...
class LeNet:
    ...
    def build(width, height, depth, classes, mode, weightsPath=None):
        ...
        if mode == 1:
            ...
            #a = Dropout(0.3)(a)
            encoded = Dense(1500, activation='sigmoid')(encoded)

            decoded = Dense(25*25*24, activation='sigmoid')(encoded)
            decoded = Reshape((25,25,24))(decoded)

            #tower_2 = Deconvolution2D(24, kernel_size=(5), strides=(5), border_mode="same", activation='relu')(decoded)
            tower_2 = Deconvolution2D(24, kernel_size=(9), border_mode="same", activation='relu')(decoded)            
            ...
            inp2 = Input(shape=(1, 1500))
            
            print "Running The Decoder!"
            decoded = Dense(25*25*24, activation='sigmoid')(inp2)
            decoded = Reshape((25,25,24))(decoded)
            ...
            model = Model(input=[inp2], output=[recustructed])
            #model = Model(input=[inp], output=[encoded])
